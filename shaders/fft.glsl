...
void main() {
    ...
    fft_re[shifted] = imageLoad(colorimg1, ivec2(invocationID)).rgb;
    fft_im[shifted] = imageLoad(colorimg2, ivec2(invocationID)).rgb;
    ...
    imageStore(colorimg1, ivec2(invocationID.xy), vec4(fft_re[shifted], 0.0));
    imageStore(colorimg2, ivec2(invocationID.xy), vec4(fft_im[shifted], 0.0));
}